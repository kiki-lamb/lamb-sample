...
void lrate() {
  if (bd_track[lrate_ix % 64] > 0) {
    bd.trigger = true;
    bd.amplitude = bd_track[lrate_ix % 64];
  }

  if (sd_track[lrate_ix % 16] > 0) {
    sd.trigger = true;
    sd.amplitude = sd_track[lrate_ix % 16];
  }

  if (sy_track[lrate_ix % 64] > 0) {
    sy.trigger = true;
    sy.amplitude = sy_track[lrate_ix % 64];
  }
